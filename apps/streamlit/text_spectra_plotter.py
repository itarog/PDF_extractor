...
def parse_proton_nmr(text: str) -> Dict:
    ...
    peaks = [item.get('shift') for item in peaks]
    return peaks
...
def parse_ir(text: str) -> List[Dict]:
    ...
    entries = [item.get('wn') for item in entries]
    return entries
...
def has_three_or_more_decimals_str(s: str) -> bool:
    """
    Return True iff s is a string representing a number that contains
    a decimal point and at least 3 digits after it.
    Examples:
        "1.234"   -> True
        "1.23"    -> False
        "1.234567"-> True
...
# 2) For numeric values: use Decimal to test exactness of 3 decimal places
def is_three_decimals_numeric(x) -> bool:
    """
    Return True iff numeric value `x` has exactly 3 decimal places.
    That means: (value * 1000) is an integer, but (value * 100) is NOT an integer.
    This treats trailing zeros as meaningful only if the numeric representation *requires*
    them; for floats we convert via str(float) to avoid binary representation artifacts.
    Examples:
        1.234   -> True
        1.23    -> False
        1.230   -> True  (float stored as 1.23 will usually be False because trailing zero lost;
                           if you need to detect that stored-as-float trailing zeros existed,
                           you must keep the original string)
        Decimal("1.230") -> True
    """
    # Convert to Decimal in a way that preserves human representation for floats:
    try:
        if isinstance(x, Decimal):
            ...
            # For floats, use str(x) so Decimal gets the human-readable form (not raw binary)
            d = Decimal(str(x))
    except (InvalidOperation, ValueError, TypeError):
        # not a numeric value that Decimal can parse
        return False

    # Multiply and check integrality using Decimal's to_integral_value (no rounding)
    times_1000 = d * Decimal(1000)
    times_100  = d * Decimal(100)

    # to_integral_value() returns the integral value without rounding change.
    is_int_1000 = times_1000 == times_1000.to_integral_value()
    is_int_100  = times_100  == times_100.to_integral_value()

    # Exactly three decimal places: divisible by 1/1000 but NOT by 1/100
    return bool(is_int_1000 and not is_int_100)
...
def parse_ms(text: str) -> List[Tuple[float, float, str]]:
    ...
    ## eliminate values without 3 digits after the point
    result = []
